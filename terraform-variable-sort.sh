#!/usr/bin/env bash
# This script sorts Terraform variables in a file in alphabetical order.
# It is idempotent, so it can be run multiple times on the same file.
# You can provide a path to the file as an argument, or it will default to variables.tf.
...
if [[ ! -f "${tf_variables_file}" ]]; then
  echo "File ${tf_variables_file} does not exist"
  exit 1
fi

cat <<- 'EOF' > /tmp/sort-tf.awk
#!/usr/bin/env -S awk -f
...
if awk --version | grep -q 'GNU Awk'; then
  awk_command=awk
elif gawk --version | grep -q 'GNU Awk'; then
  awk_command=gawk
else
  echo "GNU Awk is required"
  exit 1
fi

cat "${tf_variables_file}" | $awk_command -f /tmp/sort-tf.awk | tee "${tf_variables_file}" >/dev/null && \
rm -rf /tmp/sort-tf.awk